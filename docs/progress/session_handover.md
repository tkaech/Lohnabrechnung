...
- `Einstellungen > Berechnung` ist jetzt getrennt historisiert nach `Allgemein`, `Stundenlohn` und `Monatslohn`; offen bleibt die spaetere Aufloesung dieser drei Bereichsstaende in Monats-Snapshots und Lohnlauf
...
- die Monats-/Snapshot-Aufloesung von den drei neuen globalen Bereichsstaenden statt nur von der aktuellen `PayrollSettings`-Projektion lesen
